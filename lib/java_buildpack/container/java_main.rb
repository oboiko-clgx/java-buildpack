  ...
  module Container
    ...
    class JavaMain < JavaBuildpack::Component::BaseComponent
      ...
      def compile
        ...
        if @spring_boot_utils.is_thin?(@application)
          with_timing 'Caching Spring Boot Think Launcher Dependencies', true do
            @spring_boot_utils.cache_thin_dependencies "#{@droplet.java_home.root}/bin/java", @application.root, thin_root
          end
        end
